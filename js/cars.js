(() => {
      ...
      <article class="catalog-card ${sold ? 'sold' : ''}" data-car-url="${primaryHref}">
        <div class="catalog-card__media">
          ...
          <button class="catalog-fav ${favorite ? 'active' : ''}" type="button"
            ...
            aria-label="${t(favorite ? 'ui.removeFavourite' : 'ui.addFavourite', { model: `${car.brand} ${car.model}` })}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78Z"/></svg></button>
        </div>
  ...
  function renderChips() {
    ...
    chips.innerHTML = active.map((item) => `
      <button class="chip" type="button" data-remove-filter="${item.name}">
        ${t(`ui.filter_${item.name}`) === `ui.filter_${item.name}` ? labels[item.name] || 'Mode' : t(`ui.filter_${item.name}`)}: ${item.value} ×
      </button>`).join('');
    ...
    quickContent.innerHTML = `
      <div class="quick-layout">
        ...
        <div class="quick-layout__body">
          <span class="eyebrow">${t(`ui.${car.status}`)}</span>
  ...
  window.addEventListener('tabanji:languagechange', () => {
    [...form.elements.status.options].forEach((option) => {
      if (option.value) option.textContent = t(`ui.${option.value}`);
    });
    render();
  });
})();
