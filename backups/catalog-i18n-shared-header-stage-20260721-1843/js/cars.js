(() => {
  ...
  const labels = {
    q: 'Пошук', brand: 'Бренд', model: 'Модель', status: 'Статус',
    yearMin: 'Рік від', yearMax: 'Рік до', priceMin: 'Ціна від',
    priceMax: 'Ціна до', mileageMax: 'Пробіг до', fuel: 'Пальне',
    transmission: 'Коробка', bodyType: 'Кузов', drive: 'Привід',
    powerMin: 'Потужність від'
  };
  const statusLabels = {
    available: 'У наявності', new: 'Нове надходження', exclusive: 'Ексклюзив',
    reserved: 'Зарезервовано', sold: 'Продано', on_request: 'Під замовлення'
  };
  let visibleLimit = 8;
  let currentResults = [];
  ...
  function populateModels() {
    ...
    const models = cars
      ...
      .map((car) => car.model)
      .sort();
    form.elements.model.innerHTML = '<option value="">Усі моделі</option>';
  ...
  function cardTemplate(car) {
    ...
    const primaryText = sold ? 'Знайти схожий' : 'Переглянути автомобіль';
      ...
      <article class="catalog-card ${sold ? 'sold' : ''}">
        <div class="catalog-card__media">
          ...
          <button class="catalog-fav ${favorite ? 'active' : ''}" type="button"
            ...
            aria-label="${favorite ? 'Видалити' : 'Додати'} ${car.brand} ${car.model} ${favorite ? 'з' : 'до'} обраного">♡</button>
        </div>
        <div class="catalog-card__body">
          <span class="catalog-card__brand">${car.brand} · ${car.year}</span>
          <h3>${car.model}</h3>
          ...
          <div class="catalog-card__footer">
            ...
            <div class="card-actions">
              ...
              <button class="quick-button list-consult" type="button"
                data-modal="Запит щодо ${car.brand} ${car.model}">Консультація</button>
  ...
  function emptyTemplate() {
    ...
    const title = favoritesOnly ? 'Обране поки порожнє' : 'Точних збігів не знайдено';
    const message = favoritesOnly
      ? 'Додайте автомобілі до обраного, щоб повернутися до них пізніше.'
      : 'Змініть параметри або доручіть пошук персональному менеджеру.';
    return `
      <div class="empty-state">
        <span class="eyebrow">Private Collection</span>
        <h3>${title}</h3>
        <p>${message}</p>
        <button class="btn" type="button" data-reset-all>Повернутися до всієї колекції</button>
      </div>`;
  }
  ...
  function render({ resetLimit = false } = {}) {
    ...
    shownCount.textContent = `Показано ${visible.length} із ${currentResults.length}`;
  ...
  function renderChips() {
    ...
    if (favoriteHeader.dataset.active === 'true') active.push({ name: 'favorites', value: 'Обране' });
    chips.innerHTML = active.map((item) => `
      <button class="chip" type="button" data-remove-filter="${item.name}">
        ${labels[item.name] || 'Режим'}: ${item.value} ×
      </button>`).join('');
      ...
      <div class="quick-layout">
        ...
        <div class="quick-layout__body">
          ...
          <div class="quick-specs">
            ...
            <span>${car.drive} привід</span>
          </div>
          <p>${car.description}</p>
          <div class="card-actions">
            <a class="btn" href="car.html?id=${encodeURIComponent(car.id)}">Переглянути автомобіль</a>
            <button class="btn btn--ghost" type="button" id="quickConsult">Консультація</button>
          </div>
        </div>
      </div>`;
